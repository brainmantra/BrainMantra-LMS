...
async function run() {
  try {
    const res = await pool.query("SELECT * FROM teacher_questions WHERE level='beginner'");
    let count = 0;
    ...
    console.log(`Successfully replicated ${count} new questions from beginner to l1`);
  } catch(e) {
    console.error(e);
  } finally {
    process.exit(0);
