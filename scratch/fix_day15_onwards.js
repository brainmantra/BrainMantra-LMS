  ...
  try {
    ...
    for (const student of students) {
      ...
      const { rows: days } = await pool.query(
        `SELECT * FROM day_records WHERE student_id = $1 AND day_number >= 15 AND completed = true`,
        [studentId]
      );
    ...
    console.log(`Reset ${resetCount} records from day 15 onwards that were incompletely finished.`);
  } catch (e) {
    console.error(e);
  } finally {
    process.exit(0);
