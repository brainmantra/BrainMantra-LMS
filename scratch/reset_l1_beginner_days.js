...
async function resetL1BeginnerDays() {
  try {
    const { rows: students } = await pool.query(
      `SELECT id FROM students WHERE level IN ('l1', 'beginner')`
    );
    ...
    console.log(`Successfully reset ${updatedCount} missed day records for 24 hours for L1 & Beginner students.`);
  } catch (error) {
